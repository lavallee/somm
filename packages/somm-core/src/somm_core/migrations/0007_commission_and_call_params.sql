...
-- (superseded by correlation_id in 0009, where the column generalized
-- into a neutral hook — see somm.hooks). Auto-populated from a
-- registered correlation provider when present; NULL otherwise. This is
-- the spine that lets cross-tool audit views answer "which model calls
-- happened inside this parent task?" without joining across two
-- databases.
--
