...
### 6. Respect per-workload budgets

Every workload can carry a **daily cost ceiling** — `budget_cap_usd_daily`,
set when you register it. Treat it as a real ceiling, not a suggestion:

- When registering a workload, pass a `budget_cap_usd_daily` you'd be
  comfortable spending every day — don't leave it unbounded.
- Before adding a high-volume or fan-out call (loops, batch jobs,
  per-row LLM calls), check the workload's remaining budget via
  `somm_stats` / the rollup; back off or down-route when it's near the cap.
- Shadow-eval has its *own* separate cap — the `budget_usd_daily` key in a
  workload's shadow config — and somm stops sampling once daily eval spend
  hits it. Don't confuse the two: `budget_cap_usd_daily` bounds real calls,
  `budget_usd_daily` bounds shadow sampling.

### 7. Streaming and structured output
...
### 8. NEVER ship these patterns

These are guardrails, not style preferences. **DO NOT** weaken or remove them:

- **NEVER** import a raw provider SDK (`from anthropic import ...`) in project code.
- **NEVER** hardcode model names outside config — route via workload + provider preference.
- **DO NOT** write inline retry loops — routing handles cooldowns and fallback.
- **DO NOT** concatenate strings for long-lived prompts — use
